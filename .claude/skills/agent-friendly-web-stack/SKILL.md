...
- Use Route Handlers only for genuine HTTP boundaries such as webhooks, callbacks, public APIs, file responses, and non-UI clients. Prefer Server Actions for cookie-authenticated browser mutations. For a cookie-authenticated mutation in a Route Handler, require an `Origin` matching a configured trusted origin or validate a session-bound CSRF token; reject missing or untrusted origins when relying on origin checks. Do not use GET for application mutations.
- Match protection to the HTTP boundary: follow the provider's signature and replay-verification protocol for webhooks, preserving the raw body when it is signed; validate bearer credentials for non-cookie APIs; use the authentication SDK's state/PKCE protections for callbacks. Do not impose a blanket browser `Origin` requirement on these endpoints or public reads. Authenticate, authorize, validate, rate-limit, and make mutations idempotent as their boundary requires.
...
- Keep compiler configuration compatible with Next.js and the required lint rules. Run project scripts from the project root, retain the scaffold's package module mode, and use synchronous `babel.config.js` with `module.exports` and the `@/*` alias `${process.cwd()}/*`. Import it from `postcss.config.mjs`, which exports a named configuration object. These files need no `require()` imports or lint suppressions.
- Explicitly compile StyleX in Vitest's Vite pipeline using a compatible React/Babel integration. Reuse the Babel configuration's StyleX plugin list and preserve the project's import aliases; do not load `next/babel` into Vitest, where it can emit incompatible CommonJS. Verify a real styled component importing an aliased `.stylex.ts` token; do not mock StyleX to bypass compilation.
...
- Treat tables, views, and RPCs reachable by `anon` or `authenticated` as public [Supabase Data API boundaries](https://supabase.com/docs/guides/api/securing-your-api), even when application code uses only server clients. Direct requests enforce grants and RLS but do not execute Server Actions or their Zod checks. Enforce required integrity, transition permissions, auditing, idempotency, and abuse controls on every reachable path through database or API controls. Revoke user grants for mutations that must be exclusive to authorized server code; use the dedicated privileged client for that path.
...
- Test exposed tables and RPCs directly through the local Data API with anonymous, authorized, and other-user or other-tenant credentials. Prove that bypassing the UI and Next.js handlers cannot evade required permissions or mutation invariants. For the HTTP boundaries in use, verify legitimate requests without `Origin`, rejection of forged or unauthorized requests, and duplicate or replayed mutations producing no additional effects.
